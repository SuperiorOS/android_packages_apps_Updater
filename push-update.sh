...
updates_dir=/data/superioros_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have superior-VERSION-DATE-TYPE-* as format"
...
# Assume superior-VERSION-DATE-TYPE-*.zip
...
adb shell "killall org.superioros.updater 2>/dev/null"
adb shell "sqlite3 /data/data/org.superioros.updater/databases/updates.db" \
